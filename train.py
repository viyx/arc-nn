...
# def add_train_args(parent_parser):
    ...
    # parser.add_argument("--log_app", action='store_true') # enable logging
    # parser.add_argument("--scale_lr", action='store_true') # mult lr by num_cores as xm.optimizer sums batches grads(see https://github.com/pytorch/xla/issues/1781#issuecomment-601849130)
...
def main(cfg: DictConfig):
    assert os.environ['wandb'], 'Specify wandb login'
...
=======
        assert os.environ['XRT_TPU_CONFIG'], 'No xla device'
    wandb.login(key=os.environ['wandb'])
    xmp.spawn(map_fn, args=(cfg,), nprocs=cfg.n_cores, start_method='spawn')
>>>>>>> 848c083... add yamls
