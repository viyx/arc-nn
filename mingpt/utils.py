...
def sample_while(model, dataloader):
    end_episode_token = dataloader.dataset.end_episode_token
    model.eval()
    block_size = model.block_size
    steps = model.target_length - 1 # exclude end_episode_token
    results = []
    for (x, _) in dataloader:
        # print(x.shape)
        print(model.device)
        x = x[:, :-steps].to(model.device)
        print(x.shape)
        for step in range(steps):
            ...
            model.log('sample', step)
            # print('end')
        x = x[:,-steps:].view(-1, 30, 31).detach().cpu()
        results.append(x)
        break
        # print('break')
